...
public class AfficherEquipementsController implements Initializable {
    ...
    @FXML
    private ComboBox<String> cb_filtrage;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tf_chercher.textProperty().addListener((ObservableValue<? extends String> observable, String oldValue, String newValue) -> {
			if (!tf_chercher.getText().isEmpty()) {
                            try {
                                ObservableList data = FXCollections.observableArrayList(new Service().chercher(tf_chercher.getText()));
                                tab.setItems(data);
                            ...
                            try {
                                ObservableList data = FXCollections.observableArrayList(new Service().getAllEquipements());
                                tab.setItems(data);
                            } catch (SQLException ex) {
                                Logger.getLogger(AfficherEquipementsController.class.getName()).log(Level.SEVERE, null, ex);
                            }
         ...
         cb_filtrage.valueProperty().addListener((ObservableValue<? extends String> observable, String oldValue, String newValue) -> {
             String filtrage=cb_filtrage.getSelectionModel().getSelectedItem();
			if ((!(filtrage.equalsIgnoreCase("null")))&&(!(filtrage.equals("Toutes les equipements")))) {
                            try {
                                ObservableList data = FXCollections.observableArrayList(new Service().getEtatEquip(filtrage));
                                tab.setItems(data);
                            } catch (SQLException ex) {
                                Logger.getLogger(AfficherEquipementsController.class.getName()).log(Level.SEVERE, null, ex);
                            }
        ...
        cb_filtrage.getItems().addAll("Fonctionnel","Non fonctionnel","En maintenance","Toutes les equipements");
   ...
   doc.add(p2);
      ...
      doc.add(new Paragraph(" "));
        ...
        table.setSpacingAfter(0f);
                   ...
                   table.addCell(c1);
                  Service s=new Service();
                  if(filtrage.equalsIgnoreCase("Fonctionnel"))
                  {
                    ArrayList<Equipement> e =(ArrayList<Equipement>)s.getEtatEquip("Fonctionnel");
                   ...
                   doc.add(table);}
                  else if(filtrage.equalsIgnoreCase("Non Fonctionnel"))
                  {
                    ArrayList<Equipement> e =(ArrayList<Equipement>)s.getEtatEquip("Non Fonctionnel");
                    for(int i=0;i<e.size();i++)
                    {
                        String n=e.get(i).getLibelle();
                        c1=new PdfPCell(new Phrase(n,f4));
                   ...
                   doc.add(table);}
                  else if(filtrage.equalsIgnoreCase("En maintenance"))
                  {
                    ArrayList<Equipement> e =(ArrayList<Equipement>)s.getEtatEquip("En maintenance");
                    for(int i=0;i<e.size();i++)
                    {
                        String n=e.get(i).getLibelle();
                        c1=new PdfPCell(new Phrase(n,f4));
               ...
               Alert alerta = new Alert(Alert.AlertType.CONFIRMATION);
                         alerta.setTitle("Notification");
                         alerta.setContentText("Fichier cree avec succès");
                         alerta.showAndWait();
      ...
      btn_stat.setOnAction(new EventHandler<ActionEvent>() {
             ...
             public void handle(ActionEvent event) {
              ...
              c=  s.getNombre("En maintenance");
                 ...
                 JFreeChart chart=ChartFactory.createBarChart3D("Etat des equipements ", "Etat", "Nombres", dataset);
                 chart.setBackgroundPaint(Color.YELLOW);
                 chart.getTitle().setPaint(Color.red);
                 CategoryPlot p=chart.getCategoryPlot();
                 p.setRangeGridlinePaint(Color.BLUE);
                 ChartFrame frame=new ChartFrame("Barre d'etat des equipements",chart);
